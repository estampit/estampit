...
export function EventsFeedClient({ initial, businessId }: { initial: BusinessEventRow[]; businessId: string }) {
  const [events, setEvents] = useState<BusinessEventRow[]>(initial)
  useEffect(() => {
    const supabase = createClientComponentClient()
    const channel = supabase
      .channel('realtime-events')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'events', filter: `business_id=eq.${businessId}` }, (payload: any) => {
        setEvents(prev => [payload.new as BusinessEventRow, ...prev].slice(0, 25))
      })
      .subscribe()
